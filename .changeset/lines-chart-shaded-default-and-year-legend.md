...
Default the lines-of-code chart to age shading and name the year shades in a second legend.
...
In the by-language and by-contributor splits, a second legend below the group legend names the year bands in the base blue, so the shades no longer go unexplained; it stands off from the first legend by the chart card's own padding so the two never read as one list.
