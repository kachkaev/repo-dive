...
- **Loading**: the config lives in the analyzed repo, not in repo-insighter. Node ≥ 22.18 strips types on `import()`, so a plain dynamic `import(pathToFileURL(configPath))` of the `.ts` file should work without a bundler — verify, and fall back to also accepting `.mjs`/`.js`. Check whether Effect v4 has file-config helpers worth using (`effect/config` is env-oriented; probably not a fit — confirm against the cloned Effect source, which is the API source of truth).
- **`defineConfig` export**: needs a `repo-insighter/config` subpath export in package.json that ships types but almost no runtime (identity function). The CLI bundle currently has no secondary entry — add one to the vite config.
