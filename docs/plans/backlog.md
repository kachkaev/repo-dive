...
- **Naming**: settle `repo-insighter` vs alternatives before publicizing (see open questions). The npm name works but is a working title.
...
- **Docs drift**: specs 03/04 don't yet document the blob cache (`.repo-insighter/cache/blob-cache.sqlite`, content-addressed per-blob results) or `collectBatch`; sync them.
