...
`CHANGELOG.md` is generated by [`@changesets/changelog-github`](https://www.npmjs.com/package/@changesets/changelog-github), configured in `.changeset/config.json`. Each entry links the pull request that carried the change and the commit it was squashed into, rather than naming a bare commit hash. `disableThanks` is on, so entries carry no "Thanks @author!" credit — this is a solo repository and the attribution was only noise.

Entries carry **both** a pull request link and a commit link, which is the generator's default and what every project using it ships (astro, changesets, emotion). On a squash-merge repository the commit is derivable from the pull request, so the second link is redundant whenever a pull request exists — but it is the only anchor the older entries have, since the commits behind them predate the pull request workflow. Dropping it is not a config option either: the generator assembles its prefix as pull + commit + thanks and exposes only `repo` and `disableThanks`, so a pull-request-only format would mean maintaining a local changelog module. Considered and rejected; leave it as it is.

Looking those links up needs the GitHub API, so **`changeset version` requires `GITHUB_TOKEN` in its environment** or it fails outright. In CI this is taken care of: `changesets/action` passes the token it was given on to the `version` script. To run `pnpm changesets:version` on a laptop, provide a token with `read:user` and `repo:status` scopes yourself:

```sh
GITHUB_TOKEN=... pnpm changesets:version
```

Entries that already named a bare hash were rewritten by hand to match the generator's output. Commits from before pull requests were used carry a commit link with no pull request number, which is exactly what the generator emits when a commit has no pull request associated with it. The `0.0.1` entries never named a hash and were left as plain prose.
