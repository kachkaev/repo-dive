...
repo-dive attributes every metric to each commit's git **author** (not the committer); only the dates a snapshot is plotted at come from the committer, see [collectors](04-collectors.md#sampling).
