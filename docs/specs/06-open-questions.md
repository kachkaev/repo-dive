...
- Additive lineage composition: tree timelines currently follow a single chain, so a repository assembled from several (effect merged eight) shows only one predecessor's states before the graft, and the commit calendar — which sees every reachable commit — stays busy through stretches where the tree charts are near-empty.
  The candidate design treats every absorbed history as a **lineage component** contributing from its own root until the merge that absorbed it, and draws each tree-state series as the stepwise sum of components — which meets the post-assembly total exactly at the graft boundary (the first assembled tree _is_ the sum of the absorbed tips), fills the pre-graft years with everything that is actually in the repository, and would extend naturally to mid-life grafts.
  It cannot conjure what the repository never absorbed (effect's 2021–2023 code lived in `@effect/io`/`@effect/data`, whose histories were discarded before the monorepo assembly — only a cross-repo scan could recover that), and it needs scan/index/gc to treat absorbed chains as snapshot-worthy plus a query layer that sums stepwise series per lineage window.
