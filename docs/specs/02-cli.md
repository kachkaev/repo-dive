...
- **`ignore`** — adds the catalog folder to every ignore file at the repository root (`.gitignore`, `.prettierignore`, `.dockerignore`, …) that needs it. The catalog hides itself from git with a nested `.gitignore`, but every other tool reads a single ignore file at the root, so without this the catalog becomes their input. Each file gets one line, written the way that file is written, and files whose tool learns about the catalog elsewhere are left alone — see [config](07-config.md#catalogcheckignorefiles). `scan`, `index` and `status` warn when an entry is missing and point here; `--dry-run` reports the same plan without writing. Only existing ignore files are amended — none is created — and a catalog configured to live outside the repository needs none of this.
